...
def cross_dataset_evaluation(config, ps_model_checkpoints=None, save_dir_name=None):
    """
    ...
    if ps_model_checkpoints is not None:

        for ps_model_checkpoint in ps_model_checkpoints:
            print(f"Loading checkpoint: {ps_model_checkpoint}")
            file_name = os.path.splitext(os.path.basename(ps_model_checkpoint))[0]

            checkpoint = torch.load(ps_model_checkpoint, map_location=device)
            ...
            output_path = f'{save_dir_name}/cross_dataset_results_{timestamp}_using_{file_name}_checkpoint.json'
...
if __name__ == "__main__":
    ...
    ps_model_checkpoint = []
    # directory = "/root/Partial_Spoof_Detection_System/models/back_end_models/"
    directory = "/root/Partial_Spoof_Detection_System/models/back_end_models_no_weighted_sampling/"
    ps_model_checkpoints = [
        os.path.join(directory, f)
        for f in os.listdir(directory)]

    # save_dir_name = "outputs_with_weighted_sampling"
    save_dir_name = "outputs_no_weighted_sampling"
    results = cross_dataset_evaluation(config,ps_model_checkpoints,save_dir_name)
