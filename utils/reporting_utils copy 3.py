...
def find_hardest_samples(
    ...
    # Calculate confidence as distance from decision boundary (threshold=0.5)
    # Hardest samples = closest to threshold = lowest confidence
    # For correct predictions:
    # - Genuine (label=0, pred<0.5): confidence = 0.5 - prob (lower = closer to boundary)
    # - Spoof (label=1, pred>=0.5): confidence = prob - 0.5 (lower = closer to boundary)
    confidence = np.abs(predictions_prob - threshold)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output-dir', type=str, default='outputs/reports3',
