...
def find_hardest_samples(
    ...
    # - Genuine (label=0): confidence = |0 - prob| = prob (lower is harder)
    # - Spoof (label=1): confidence = |1 - prob| = 1 - prob (lower is harder)
    confidence = np.where(labels == 0, 
                          predictions_prob,        # Genuine: lower prob = harder
                          1 - predictions_prob)    # Spoof: lower prob = harder
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output-dir', type=str, default='outputs/reports2',
